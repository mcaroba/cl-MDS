...
def _smacof_single(dissimilarities, metric=True, n_components=2, init=None,
    ...
    if weights is not None:
        assert weights.shape == dissimilarities.shape
    ...
    for it in range(max_iter):
        ...
        if weights is None:
            ...
            V = -weights
            u = np.sum(weights, axis=1) - np.diagonal(weights)
            np.fill_diagonal(V,u)
            V_inv = np.linalg.pinv(V, hermitian=True)
            X = np.dot(V_inv, np.dot(B, X))
...
class MDS(BaseEstimator):
    ...
    def fit_transform(self, X, y=None, init=None, weights=None):
        ...
        self.embedding_, self.stress_, self.n_iter_ = smacof(
            ...
            return_n_iter=True, weights=self.weights_)
